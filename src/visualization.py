"""
Visualization module for card game detection system.
...
class Visualizer:
    ...
    def __init__(self, output_dir: str = "output_visualization"):
    ...
    def draw_scoreboard(self, frame: np.ndarray, scores: Dict[str, int], width: int):
    ...
    def draw_events(self, frame: np.ndarray, events: List[str], width: int):
        ...
        self.draw_text_with_bg(frame, event, (x, y), (255, 255, 255), 
                              font_scale=0.8, bg_alpha=0.8)
    
    def draw_token_messages(self, frame: np.ndarray, messages: Dict[int, str], 
        ...
        for battle_id, message in messages.items():
            ...
            self.draw_text_with_bg(frame, message, (x, y_offset), (255, 100, 255), 
                                  font_scale=0.6, bg_alpha=0.8)
            y_offset += 30
    
    def draw_board(self, frame: np.ndarray, board_corners: np.ndarray):
    ...
    def draw_cards(self, frame: np.ndarray, cards: List):
        ...
        for card in cards:
            ...
            # Draw label
            top = self.get_top_point(card.box)
            label = f"Card {card.card_id}"
            if card.team:
                label += f" ({card.team})"
            
            (tw, th), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
            self.draw_text_with_bg(frame, label, (top[0]-tw//2, top[1]-10), 
                                  color, font_scale=0.6)
    
    def draw_battles(self, frame: np.ndarray, battles: List, token_detector=None):
        ...
        for battle in battles:
            ...
            # Draw battle label
            top = self.get_top_point(battle.box)
            text = f"Battle: {battle.card1_id}({battle.team1}) vs {battle.card2_id}({battle.team2})"
            
            (tw, th), _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)
            self.draw_text_with_bg(frame, text, (top[0]-tw//2, top[1]-10), BATTLE_COLOR)
    
    def draw_stats(self, frame: np.ndarray, frame_count: int, total_frames: int, 
    ...
    def save_detection_visualization(self, frame: np.ndarray, gray: np.ndarray, 
                                    ...
                                    markers_pre: np.ndarray, markers_post: np.ndarray,
                                    params, detected_cards: List):
        """Save 9-step watershed visualization"""
        self.frame_number += 1
        ...
        labels = [
            ["1. Grayscale", f"2. Blur {params.blur_size}x{params.blur_size}", 
             f"3. Canny {params.canny_low}/{params.canny_high}"],
            [f"4. Dilate {params.edge_dilate_size}x{params.edge_dilate_size}x{params.edge_dilate_iterations}", 
             "5. Mask", "6. Distance"],
            [f"7. FG thresh={params.distance_threshold}", 
             f"8. BG iter={params.background_iterations}", "9. Unknown"]
        ]
        ...
        cv2.putText(summary_panel, "9-STEP SIMPLIFIED WATERSHED (HARSH PARAMETERS)", 
                   (20, 25), font, 0.7, (0, 255, 255), 2)
        cv2.putText(summary_panel, f"Frame {self.frame_number} | Detected: {num_cards_detected} cards", 
                   (20, 50), font, 0.6, (255, 255, 255), 1)
        cv2.putText(summary_panel, 
                   f"Heavy blur={params.blur_size}x{params.blur_size} removes internals | " +
                   f"Minimal dilate={params.edge_dilate_size}x{params.edge_dilate_size}x{params.edge_dilate_iterations} preserves gaps", 
                   (20, 73), font, 0.45, (200, 200, 200), 1)
        cv2.putText(summary_panel, 
                   f"High thresh={params.distance_threshold} for confident centers | " +
                   f"Less bg_expand={params.background_iterations} for larger unknown region", 
                   (20, 92), font, 0.45, (150, 150, 255), 1)
        ...
        print(f"✓ Saved: {output_path} ({num_cards_detected} cards)")
